...
describe('TmuxManager', () => {
  ...
  describe('createWindow', () => {
    ...
    it('escapes session target when creating windows', () => {
      tmux.createWindow("agent-session'; touch /tmp/pwn", 'my-window');

      const lastCommand = executor.getLastCommand();
      expect(lastCommand).toContain("'agent-session'\\''; touch /tmp/pwn'");
    });
  });
  ...
  describe('sendKeysToWindow', () => {
    ...
    it('escapes combined session and window target', () => {
      tmux.sendKeysToWindow("agent-session'; touch /tmp/pwn", 'window1', 'echo hello');

      expect(executor.calls[0].command).toContain("'agent-session'\\''; touch /tmp/pwn:window1'");
    });
  });
  ...
  describe('capturePaneFromWindow', () => {
    it('returns captured pane content', () => {
      ...
      expect(executor.getLastCommand()).toContain('-S -5000');
    });
  });
  ...
  describe('ensureAgentInWindow', () => {
    it('creates and starts the agent when the window is missing', () => {
      executor.nextResult = 'zsh\n';

      tmux.ensureAgentInWindow('agent-session', 'codex', 'cd "/repo" && codex');

      expect(executor.calls[0].command).toContain("tmux list-windows -t 'agent-session'");
      expect(executor.calls[1].command).toContain("tmux new-window -t 'agent-session' -n 'codex'");
      expect(executor.calls[2].command).toContain("tmux send-keys -t 'agent-session:codex'");
      expect(executor.calls[2].command).toContain('codex');
    });

    it('does not inject a new command when the agent window already exists', () => {
      executor.nextResult = 'zsh\ncodex\n';
