...
export class TmuxManager {
  ...
  createWindow(sessionName: string, windowName: string): void {
    const escapedSessionName = this.escapeShellArg(sessionName);
    const escapedWindowName = this.escapeShellArg(windowName);

    try {
      this.executor.exec(`tmux new-window -t ${escapedSessionName} -n ${escapedWindowName}`);
    } catch (error) {
  ...
  listWindows(sessionName: string): string[] {
    try {
      const escapedSessionName = this.escapeShellArg(sessionName);
      const output = this.executor.exec(`tmux list-windows -t ${escapedSessionName} -F "#{window_name}"`);
  ...
  sendKeysToWindow(sessionName: string, windowName: string, keys: string): void {
    ...
    const escapedTarget = this.escapeShellArg(target);
    const escapedKeys = this.escapeShellArg(keys);
  ...
  capturePaneFromWindow(sessionName: string, windowName: string): string {
    ...
    try {
      return this.executor.exec(`tmux capture-pane -t ${escapedTarget} -p -S -5000`);
    } catch (error) {
  ...
  /**
   * Ensure an agent window exists and start it only when missing.
   */
  ensureAgentInWindow(sessionName: string, windowName: string, agentCommand: string): void {
  ...
  setSessionEnv(sessionName: string, key: string, value: string): void {
    ...
    try {
      this.executor.exec(`tmux set-environment -t ${escapedSessionName} ${escapedKey} ${escapedValue}`);
    } catch (error) {
      throw new Error(
